...
from lumensigner.hardware.buttons import HardwareButtonsConstants, HardwareButtons
from lumensigner.models.encode_qr import EncodeQR
from lumensigner.models.settings import SettingsConstants
from lumensigner.models.threads import BaseThread, ThreadsafeCounter
...
class ButtonListScreen(BaseTopNavScreen):
    button_data: list = (
        None  # list can be a mix of str or tuple(label: str, icon_name: str)
    )
    ...
    def __post_init__(self):
        ...
        if len(self.button_data) == 1:
            ...
            button_list_height = (len(self.button_data) * button_height) + (
                GUIConstants.COMPONENT_PADDING * (len(self.button_data) - 1)
            )
        ...
        for i, button_label in enumerate(self.button_data):
            ...
            button_kwargs = dict(
                ...
                scroll_y=self.scroll_y_initial_offset
...
class LargeButtonScreen(BaseTopNavScreen):
    ...
    def __post_init__(self):
        ...
        if len(self.button_data) == 2:
            button_start_y = self.top_nav.height + int(
                (
                    self.canvas_height
        ...
        while True:
            ...
            with self.renderer.lock:
                if user_input == HardwareButtonsConstants.KEY_UP:
                    ...
                    user_input == HardwareButtonsConstants.KEY_RIGHT
                    ...
                    and not self.top_nav.show_power_button
                ):
                    self.top_nav.is_selected = False
                    self.top_nav.render_buttons()
...
class QRDisplayScreen(BaseScreen):
    ...
    def __post_init__(self):
        from lumensigner.models.settings import Settings
    ...
    * initial_value: initialize the TextEntryDisplay with an existing string
    ...
    def _run(self):
        ...
        while True:
            ...
            if (
                input
                in [HardwareButtonsConstants.KEY_UP, HardwareButtonsConstants.KEY_DOWN]
                and self.top_nav.is_selected
            ):
